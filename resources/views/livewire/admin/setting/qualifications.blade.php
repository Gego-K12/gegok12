      ...
      <section class="section">
        <div class="w-full bg-white shadow-lg rounded-xl border overflow-hidden">
            <div class="flex items-center justify-between px-6 py-4 border-b bg-gray-50">
			    <h1 class="admin-h1">Qualification</h1>

			    <a href="{{ url('admin/setting/qualification/create') }}"
                    ...
                    <table class="table table-bordered borderTable">
                        ...
                        @if(count($qualifications) > 0)
                            @foreach($qualifications as $qualification)
                                <tbody>
                                    <td>
                                        {{ ucfirst($qualification->display_name) }}
                                    </td>
                                    <td>
                                        {{ ucfirst($qualification->type) }}
                                    </td>
                                    <td>
                                         <span class="px-2 py-1 text-xs rounded
										    {{ $qualification->status == 0 ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">
										    {{ $qualification->status == 0 ? 'In-active' : 'Active' }}
										</span>
                                    </td>
                                    <td class="py-3 px-2">
                                        <div class="flex items-center">
										    <a href="{{ route('admin.setting.qualifications.update', ['id' => $qualification->id]) }}" title="Edit">
