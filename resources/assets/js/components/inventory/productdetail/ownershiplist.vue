<template>
   <div class="px-3 overflow-x-scroll lg:overflow-x-auto md:overflow-x-auto py-3" v-bind:class="[this.product_tab==2?'block' :'hidden']">
  <div class="relative">
    ...
    <div class="flex flex-wrap lg:flex-row justify-between items-center">
      <div class="">
        <h1 class="admin-h1 my-3">Product Ownership</h1>
      </div>
        ...
        <table class="w-full">
          <thead class="bg-grey-light">
            <tr class="border-b">
              ...
              <th class="text-left text-sm px-2 py-2 text-grey-darker">Product Unique code</th>
              <th class="text-left text-sm px-2 py-2 text-grey-darker" > Ownership_member </th>
          ...
          <tbody v-if="this.productownerships != ''" >
            <tr class="border-b" v-for="(productownership,k1) in productownerships" :key="k=k1+1">
              <td class="py-3 px-2" >
                <p class="font-semibold text-xs">{{k}}</p>
              </td>
              <td class="py-3 px-2" >
                <p class="font-semibold text-xs">{{productownership.productcode.code}}</p>
              </td>
              <td class="py-3 px-2">
                <p class="font-semibold text-xs">{{productownership.user.name}}
                  
                </p>
              </td>
              <td class="py-3 px-2">
                <p class="font-semibold text-xs">{{productownership.start_date}}
                  
                </p>
              </td>
              <td class="py-3 px-2">
                <p class="font-semibold text-xs">{{productownership.end_date}}
             ...
             <!--  <td class="py-3 px-2">
                <div class="flex items-center">                
             <a @click="productownerEdit(productownership.id)" class="cursor-pointer" title="Edit">
    ...
    C443.754,73.631,440.467,81.575,434.603,87.419z"></path></g></g></svg>
                  ...
                  <a href="#" @click="deleteproductownership(productownership.id)" class="cursor-pointer" title="Delete">
          ...
          <div class="modal-body" >
            ...
            <div class="flex items-center">
              ...
              <div class="my-1 w-full">
                ...
                <span v-if="errors.owner_id" class="text-red-500 text-xs font-semibold">{{errors.owner_id[0]}}</span>
              </div>
...
<!-- 
                    <div>
                    <label class="tw-form-label ">End date</label>
                    <div class="my-1">
                     <input v-model="end_date" name="end_date"  value="end_date" type="date" class="tw-form-control w-full"><span v-if="errors.end_date" class="text-red-500 text-xs font-semibold">{{errors.end_date[0]}}</span>
                     </div>
                    </div> -->
...
export default {
  ...
  methods:{
    getlist(page=1)
    {
       axios.get('/admin/productowner/'+this.productid+'/list?page='+this.page).then(response=>{
                  console.log(response);
                  this.productownerships=response.data.product_ownership.data;
                  this.page_count = response.data.product_ownership.last_page;
                  this.total = response.data.product_ownership.total;
    ...
    addproductowner()
    {
      ...
      this.errors=[];
                 axios.post("/admin/productowner/add",{
                    product_unique_code:this.productid,
    ...
    deleteproductownership(id) 
    {
      ...
      swal({
        ...
        if (isConfirm) 
        {
          axios.delete('/admin/productowner/'+id+'/delete').then(response => {
