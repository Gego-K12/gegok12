...
@section('content')
    <!-- <div class="w-full">
        <div>
            <h1 class="admin-h1 my-3 flex items-center">
                <a href="{{ url('/admin/video-conference/manage-invites/'.$conference->id) }}" title="Back" class="rounded-full bg-gray-100 p-2">
        ...
        <div class="bg-white shadow px-4 py-3">
            ...
            <form method="post" action="{{ url('admin/video-conference/save-invites/'.$conference->id) }}" id="stream" enctype="multipart/form-data">
                @csrf  
                <div class="my-3">
                    <div class="">
                                ...
                                <select name="students[]" class="mdb-select md-form w-full" multiple>
                                    ...
                                    @if(count($student)>0)
                                        @foreach($student as $userinfo)
                                            ...
                                            @endif
                                            <option value="{{$userinfo->id}}" {{$checkExist}}>{{$userinfo->name.' ('.$userinfo->email.')'}}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
    ...
    <add-invites url="{{ url('/') }}" conference_id="{{$conference->id}}" ></add-invites>
@endsection