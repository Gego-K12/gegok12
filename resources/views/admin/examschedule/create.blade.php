...
@section('content')
    <div class="">
        <h1 class="font-semibold text-gray-700 my-3 flex items-center text-3xl">
            <a href="{{ url('/admin/exam') }}" title="Back" class="rounded-full bg-gray-100 p-2">
            ...
            <span class="mx-3">Add Schedule</span>
        </h1>

        <h2 class="text-lg font-semibold mb-5 flex items-center">
            <span class="mx-3">Exam Name : {{ $exam->name }}</span>
            <span class="mx-20">Section : {{ $exam->standardlink->StandardSection }}</span>
        </h2>
        @include('partials.message')
        <create-examschedule  id="{{ $exam->id }}" url="{{ url('/') }}"></create-examschedule>  
    </div>
@endsection