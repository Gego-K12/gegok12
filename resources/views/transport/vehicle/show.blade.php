...
<span class="mx-3">Vehicle</span>
	...
	<div class="flex border shadow py-3 px-5 text-white justify-between items-center">
		...
		<div class="flex-col">
			<p class="text-gray-700 text-2xl font-medium">{{ucfirst($vehicle->name)}}</p>
...
<g>
		...
		<circle cx="192" cy="341.333" r="42.667"/>
			...
			<div id="product-option" class="bg-white shadow rounded hidden absolute right-0 w-20">
				<ul class="list-reset text-sm text-gray-800 leading-relaxed">
					<li class="px-2 py-1"><a href="{{url('/admin/transport/vehicle/'.$vehicle->id.'/edit')}}" class="hover:text-gray-600">Edit</a></li>
					<li class="px-2 py-1"><a href="#" rel="{{url('/admin/transport/vehicle/'.$vehicle->id.'/delete')}}"  class="hover:text-gray-600 delete">Delete</a></li>
				</ul>
			</div>
		</div>
		<!-- end -->
  ...
  <div class="bg-white shadow my-5">
   <vehicle-tab url="{{ url('/') }}" vehicleid="{{$vehicle->id}}"  ></vehicle-tab>
                </div>
               ...
               $.ajax({
                  ...
                  success:function(data)
                  {
                     swal({
                        ...
                        window.location.assign('/product/show');
                     });
                  }
               })
            }
