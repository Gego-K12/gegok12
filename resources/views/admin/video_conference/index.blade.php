...
@section('content')
    <div class="py-3">
        <div class="flex justify-between mb-4 items-center">
            <h2 class="admin-h1 my-3">Video Room</h2>
            <form action="{{ url('/admin/video-conference') }}" enctype="multipart form-data">
                <div class="">
                    <div class="flex items-center mx-2">
                        ...
                        <div class="date-select date-select_none dashboard-reset mx-1 lg:mx-0 md:mx-0">
                            <a href="{{ url('/admin/video-conference') }}" id="do-reset" class="text-sm border bg-gray-100 text-grey-darkest py-1 px-4">Reset</a>
                        </div>
                    </div>
                </div>
            </form>
            <a href="{{url('/admin/video-conference/create')}}" class="no-underline text-white  px-4 my-3 mx-1 flex items-center custom-green py-1 justify-center">
                ...
                <div class="flex flex-col bg-white border shadow p-3 mb-4">
                    <div class="flex justify-between">
                        <div class="uppercase tracking-wide text-sm text-indigo-600 font-bold">
                            {{ $stream->name }} <span class="mx-3 text-xs bg-indigo-600 text-white px-2 py-1 rounded-full font-medium">{{ $stream->status }}</span>
                        </div>
                        <div class="">
                            @if($stream->user_id==Auth::user()->id && $stream->status!='stop')
                                <a href="{{ url('admin/video-conference/edit/'.$stream->id) }}" class="btn bg-green-600 text-white rounded px-3 py-1">Edit</a>
                    ...
                    <div class="flex justify-start">
                        ...
                        @if($stream->status!='stop')
                            <a class="block text-center btn px-4 py-1 rounded bg-green-600 text-white" href="{{url('admin/video-conference/'.$stream->slug)}}">Go to Room</a>
                        @endif
                        @endif

                        @if($stream->compose_status=='available')
                            <a class="block text-center btn px-4 py-1 rounded bg-green-600 text-white" href="{{url('admin/video-conference/recordings/'.$stream->id)}}">Go to Recordings</a>
                        @endif
                        @if($stream->status!='stop')
                            <a class="ml-4 block text-center btn px-4 py-1 rounded bg-green-600 text-white" href="{{url('admin/video-conference/manage-invites/'.$stream->id)}}">Manage Invitees</a>
                        @endif
                    </div>
                </div>
            @endforeach
