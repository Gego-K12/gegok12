            ...
            <div class="row justify-content-center">
                <div class="flex flex-wrap lg:flex-row items-center">
                    <div class="flex items-center justify-between w-full">
                        <h1 class="admin-h1 my-3">Sales List</h1>
                ...
                <div class="flex flex-wrap custom-table my-3 overflow-auto">
                    <table class="w-full">
                        <thead>
                            <tr class="bg-grey-light">
                                ...
                                <th class="text-left text-sm px-2 py-2 text-grey-darker">Sales Date</th>
                               
                                <th>Action</th>
                            </tr>
                        </thead>
                        <tbody v-if="this.sales != ''">
                            <tr v-for="sales in sales">
                                <td><a :href="url+'/'+mode+'/sales/'+sales.id+'/show'">{{sales.product_name}}</a></td>
                                <td>{{sales.standard}}</td>
                                ...
                                <td>
                                    <div class="flex items-center">                
                                        <a :href="url+'/'+mode+'/sales/'+sales.id+'/edit'" class="cursor-pointer" title="Edit">
                                        ...
                                        <a :href="url+'/'+mode+'/sales/'+sales.id+'/show'" target="_blank" class="cursor-pointer" title="Show">
                                        ...
                                        <a @click="deletesales(sales.id)" class="cursor-pointer" title="Delete">
                    ...
                    <paginate v-model="page" :page-count="this.page_count" :page-range="3" :margin-pages="1" :click-handler="salesshow" :prev-text="'<'" :next-text="'>'" :container-class="'pagination'" :page-class="'page-item'" :prev-link-class="'prev'" :next-link-class="'next'"></paginate>
                </div>
            </div>
        </div>
    </div>
...
<script>
    export default {
        ...
        methods:
        {
            salesshow(page=1)
            {
                axios.get(this.url+'/'+this.mode+'/sales/list?page='+this.page+'&'+this.searchquery).then(response=>{
            ...
            deletesales(id) 
            {
                ...
                swal({
                    ...
                    if (isConfirm) 
                    {
                        axios.delete(thisswal.url+'/'+thisswal.mode+'/sales/'+id+'/delete').then(response => {
            ...
            searchSales()
            {
                ...
                this.final=this.url+'/'+this.mode+'/sales/show?search='+this.searchquery;
            ...
            resetForm()
            {
                window.location.href=this.url+'/'+this.mode+'/sales/show';
            }
        },
