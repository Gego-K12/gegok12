  ...
  <div class="px-3 overflow-x-scroll lg:overflow-x-auto md:overflow-x-auto py-3" v-bind:class="[this.quiz_tab==1?'block' :'hidden']">
    <div class="relative">
      ...
      <div class="flex flex-wrap lg:flex-row justify-between items-center my-1">
        ...
        <div class="relative flex items-center w-8/12 lg:w-2/5 md:w-2/5 justify-end">

          <div class="lg:mr-3 md:mr-3">
            <div class="">
              <select name="head_id" id="head_id" v-model="head_id" class="tw-form-control w-full" @change="listChange()">
           ...
           <div class="lg:mr-3 md:mr-3">
            <div class="">
              <select name="type_id" id="type_id" v-model="type_id" class="tw-form-control w-full" @change="listChange()">
          ...
          <div class="lg:mr-3 md:mr-3">
            <div class="">
              <select name="page_no" id="page_no" v-model="page_no" class="tw-form-control w-full" @change="listChange()">
                <option value="" >All pages</option>
                <option v-for="pagenum in pagenums" v-bind:value="pagenum">{{pagenum}}</option>
              </select>
            </div>
          </div> 

          <a @click="resetFilter()" dusk="reset-btn" class="btn btn-reset bg-gray-100 text-gray-700 border rounded px-3 py-1 mr-3 text-sm font-medium cursor-pointer">Reset</a>

          <div class="flex items-center"> 
            <a :href="url+'/chapter/'+chapter_id+'/question/'"  class="no-underline text-white px-4 mx-1 flex items-center custom-green py-1 justify-center">
          ...
          <table class="w-full">
            <thead class="bg-grey-light">
              <tr class="border-b">
                ...
                <th class="text-left text-sm px-2 py-2 text-grey-darker"> Questions</th>
                <th class="text-left text-sm px-2 py-2 text-grey-darker"> Head </th>
                <th class="text-left text-sm px-2 py-2 text-grey-darker"> Type </th>
                <th class="text-left text-sm px-2 py-2 text-grey-darker"> Page no </th>
            ...
            <tbody v-if="this.questions != ''">
              <tr class="border-b" v-for="(question,k1) in questions" :key="k=k1+1"  >
                ...
                <td class="py-3 px-2">
                  <div class="font-semibold text-xs" v-html="question.question"></div>
                </td>
                <td class="py-3 px-2">
                  <p class="font-semibold text-xs">{{question.head}}</p>
                </td>
                <td class="py-3 px-2">
                  <p class="font-semibold text-xs">{{question.typename}}</p>
                </td>
                <td class="py-3 px-2">
                  <p class="font-semibold text-xs">{{question.page_no}}</p>
                </td>
                <td class="py-3 px-2">
                  <div class="flex items-center">
                  <a class="cursor-pointer" @click="showQuestion(question.id)"  title="View">
                    ...
                    <a :href="url+'/test/question/'+question.id+'/edit'"  title="Edit">
                    ...
                    <a class="cursor-pointer" @click="deletequestion(question.id)" title="Delete">
     ...
     <div v-if="this.show == 1" class="modal modal-mask">
      <div class="modal-wrapper px-4">
        ...
        <div class="px-2 flex py-1 text-black">
          ...
          <h3 class="justify-center question text-base" v-html="this.question.question"></h3>
        </div>
      </div>
<div  v-show="this.question.quizoptions!=''">
  <div class="w-full lg:w-1/4"> 
                                <label for="mark" class="tw-form-label">Options<span class="text-red-500">*</span></label>
                            </div>
       <div class="flex justify-center mx-2 w-3/4 text-sm" v-for="opt in question.quizoptions">
...
<script>
  ...
  export default {
    props:['url','chapter_id','subject_id'],
    data () {
      return {
        page_count:0,
        page:0,
    ...
    methods:
    {
      getlist()
      {
        axios.get('/test/question/'+this.chapter_id+'/list?page='+this.page+'&type='+this.type_id+'&head='+this.head_id+'&page_no='+this.page_no).then(response => {
      ...
      {
        this.show=1;
        axios.get('/test/question/'+id+'/show').then(response => {
      ...
      deletequestion(id) 
      {
        ...
        swal({
          ...
          if (isConfirm) 
          {
            axios.delete(thisswal.url+'/test/question/'+id+'/delete').then(response => {
