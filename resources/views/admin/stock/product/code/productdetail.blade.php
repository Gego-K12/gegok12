...
<a href="{{url('/admin/product/'.$product->product->id.'/show')}}" title="Back" class="rounded-full bg-gray-100 p-2">
...
<span class="mx-3">Productcode</span>
</h1>
</div>
<div class="bg-white shadow my-5">
	<div class="flex border shadow py-3 px-5 text-white justify-between">
		...
		<div class="flex-col">
			<p class="text-gray-700 text-2xl font-medium">{{$product->code}}</p>
  ...
  <div class="bg-white shadow my-5">
   <product-tab url="{{ url('/') }}" productid="{{$product->id}}" ownership_status="{{$product->product->ownership_tracking}}" track_status="{{$product->product->location_tracking}}" condition_status="{{$product->product->maintainence_tracking}}" product_id="{{$product->product->id}}" ></product-tab>
