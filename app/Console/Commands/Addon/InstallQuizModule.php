...
class InstallQuizModule extends Command
{
    protected $signature = 'module:install-quiz';
    ...
    public function handle()
    {
        try{
           ...
           /* if (!$repo || !$package || !$username || !$token) {
            ...
            foreach (['quiz-routes', 'quiz-components', 'quiz-views', 'quiz-migrations', 'quiz-config'] as $tag) {
            ...
            // if (!str_contains(file_get_contents($appJsPath), "require('./gquiz')")) {
            ...
            $importLine = "import { registerQuiz } from './gquiz'";
            ...
            if (!str_contains($content, "registerQuiz(app)")) {
