<template>
   <div class="px-3 overflow-x-scroll lg:overflow-x-auto md:overflow-x-auto py-3" v-bind:class="[this.vehicle_tab==2?'block' :'hidden']">
  <div class="relative">
    ...
    <div class="flex flex-wrap lg:flex-row justify-between items-center">
      <div class="">
        <h1 class="admin-h1 my-3">Driver List</h1>
      </div>
      ...
      <div class="flex flex-wrap custom-table my-3 overflow-auto">
        <table class="w-full">
          <thead class="bg-grey-light">
            <tr class="border-b">
              <th class="text-left text-sm px-2 py-2 text-grey-darker" width="10%"> No </th>
              <th class="text-left text-sm px-2 py-2 text-grey-darker" width="40%">Name</th>
               <th class="text-left text-sm px-2 py-2 text-grey-darker" width="50%">Assign_at</th>
          ...
          <tbody v-if="this.users != ''" >
            <tr class="border-b" v-for="(user,k1) in users" :key="k=k1+1">
              ...
              <td class="py-3 px-2" >
                <p class="font-semibold text-xs">{{user.user.userprofile.firstname}} {{user.user.userprofile.lastname}} </p>
              </td>
               <td class="py-3 px-2" >
                <p class="font-semibold text-xs">{{user.created_at}}</p>
              </td>
             <!--  <td class="py-3 px-2">
                <p class="font-semibold text-xs">{{locationproduct.productcode.code}}
                  
                </p>
              </td>
              <td class="py-3 px-2">
                <p class="font-semibold text-xs">{{locationproduct.start_date}}
                  
                </p>
              </td>
              <td class="py-3 px-2">
                <p class="font-semibold text-xs">{{locationproduct.end_date}}
                  
                </p>
              </td>
              <td class="py-3 px-2">
                <p class="font-semibold text-xs">{{locationproduct.remarks}}
             ...
             <!--  <td class="py-3 px-2">
                <div class="flex items-center">                
             <a @click="locationproductEdit(locationproduct.id)" class="cursor-pointer" title="Edit">
    ...
    C443.754,73.631,440.467,81.575,434.603,87.419z"></path></g></g></svg>
                  ...
                  <a href="#" @click="deletelocationproduct(locationproduct.id)" class="cursor-pointer" title="Delete">
            ...
            <div class="">
              ...
              <div class="my-1">
                ...
                <div >
                     ...
                     <div class="my-1">
                      <textarea v-model="editremarks" name="editremarks"  value="editremarks" type="textarea" placeholder="remarks" class="tw-form-control w-full"></textarea><span v-if="errors.remarks" class="text-red-500 text-xs font-semibold">{{errors.remarks[0]}}</span>
                      </div>
                    </div>
...
components: {
  ...
  methods:{
    getlist(page=1)
    {
       axios.get('/admin/transport/vehicle/driver/'+this.vehicleid+'/show?page='+this.page).then(response=>{
    ...
    addlocationproduct()
    {
      ...
      this.errors=[];
                 axios.post("/admin/locationproduct/add",{
                    ...
                    product_unique_code:this.vehicleid,
    ...
    deletelocationproduct(id) 
    {
      ...
      swal({
        ...
        if (isConfirm) 
        {
          axios.delete('/admin/locationproduct/'+id+'/delete').then(response => {
