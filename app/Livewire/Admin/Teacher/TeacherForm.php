...
class TeacherForm extends Component
{
    ...
    public function mount($name = null)
    {
        ...
        $this->designationlist = SiteHelper::getTeachingDesignations();
        ...
        $this->customFields = CustomFieldHelper::getFieldsForEntity('teacher', $schoolId);
        ...
        $user = TeacherUser::where('name', $name)->firstOrFail();

        abort_unless(Gate::allows('member', $user), 403);

        $userprofile = Userprofile::where('user_id', $user->id)->first();
        $teacherprofiles = TeacherProfile::where('user_id', $user->id)->get();
        $firstProfile = $teacherprofiles->first();
        ...
        if ($teacherprofiles->isNotEmpty()) {
            $this->qualifications = $teacherprofiles->pluck('qualification_id')->map(fn($id) => $id ?? '')->all();
        }

        $storedCustomFieldValues = CustomFieldHelper::getValues('teacher', $user->id);
    ...
    protected function rules(): array
    {
        ...
        $aadharRule = Rule::unique('userprofiles', 'aadhar_number')->where(fn($query) => $query->where('school_id', $schoolId)->whereIn('usergroup_id', [5, 8, 10, 11, 13]));
        if ($this->isEdit()) {
        ...
        if ($this->isEdit()) {
            ...
            $rules['mobile_no'] = ['required', 'numeric', 'digits:10', Rule::unique('users', 'mobile_no')->where(fn($query) => $query->where('usergroup_id', 5))];
        ...
        $rules = array_merge($rules, CustomFieldHelper::validationRules('teacher', $schoolId));
    ...
    {
        return CustomFieldHelper::validationAttributes('teacher', Auth::user()->school_id);
    }
    ...
    public function save()
    {
        ...
        if ($this->isEdit()) {
            ...
            $this->redirect(url('/admin/teacher/edit/' . $this->teacherName), navigate: false);
        ...
        $message = trans('messages.add_success_msg', ['module' => 'Teacher']);
        ...
        $this->redirect(url('/admin/teachers'), navigate: false);
    }

    protected function persistCustomFields(int $userId, int $schoolId): void
    {
        foreach (CustomFieldHelper::getFieldsForEntity('teacher', $schoolId) as $field) {
            ...
            CustomFieldHelper::setValue($field->id, 'teacher', $userId, $schoolId, $value);
        }
    }

    public function render()
    {
        return view('livewire.admin.teacher.teacher-form');
    }
}
