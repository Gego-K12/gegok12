...
trait PaymentProcess
{
     ...
     * @param  Subscription  $payment  Existing subscription/payment record
     * @return void
     */
    public function PaymentProcess($data, $user_id, $school_id, $payment)
    {
        ...
        try {
            if (($data->status == 'success') && ($payment->status == 'pending')) {
                ...
                $payment->payment_details = ['merchant_key' => $data->key,
                    'txnid' => $data->txnid,
                    'amount' => $data->amount,
                ...
                $payment->plan_details = ['no_of_members' => $plan->no_of_members,
