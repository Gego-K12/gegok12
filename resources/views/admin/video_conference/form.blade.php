...
@section('content')
    <!-- <div class="w-full">
        ...
        <div class="bg-white shadow px-4 py-3">
            ...
            <form method="post" action="{{ url('admin/video-conference/save') }}" id="stream" enctype="multipart/form-data">
                @csrf
                ...
                <div class="my-3">
                    <div class="">
                        <div class="w-full lg:w-1/4">
                            <label for="description" class="tw-form-label">Select Staff</label>
                        </div>
                        <div class="w-full lg:w-2/5 my-2">
                                ...
                                <select name="students[]" class="mdb-select md-form w-full" multiple>
                                    ...
                                    @if(count($student)>0)
                                        @foreach($student as $userinfo)
                                            <option value="{{ $userinfo->id }}">{{ $userinfo->FullName.' ('.$userinfo->email.')'}}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
    ...
    <create-conference url="{{ url('/') }}" date="{{ date('d-m-Y H:i:s') }}" ></create-conference>
@endsection