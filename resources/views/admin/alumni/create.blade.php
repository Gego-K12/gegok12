...
@section('content')
    <div class="w-full lg:mx-2">
        <h1 class="admin-h1 my-3 flex items-center">
            <a href="{{ url('/admin/alumni') }}" class="rounded-full bg-gray-100 p-2" title="Back">
            ...
            <span class="mx-3">Add Alumni</span>
        </h1>
        @include('partials.message')
        @if($count < $subscription->plan->no_of_members)
            <form method="POST" action="" enctype="multipart/form-data">
                @csrf
                <create-alumni url="{{ url('/') }}"></create-alumni>
            </form>
        @else
            <a href="{{ url('/pricing') }}"> 
                <button type="submit" class="no-underline text-white  px-4 my-3 mx-1 flex items-center custom-green py-1 justify-center">Upgrade Plan to Add More Alumni</button>
            </a>
        @endif
    </div>
@endsection