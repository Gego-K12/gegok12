            ...
            <h1 class="admin-h1 my-3 flex items-center">
                ...
                <div class="relative flex items-center w-3/4 lg:justify-end">
                    <div class="flex items-center" dusk="add-button">
                        <a href="{{ url('admin/video-conference/add-invites/'.$conference->id) }}" class="no-underline text-white  px-4 my-3 mx-1 flex items-center custom-green py-1 justify-center">
                        ...
                        <table class="w-full">
                            <thead class="bg-grey-light">
                                <tr class="border-b">
                                    ...
                                    <th class="text-left text-sm px-2 py-2 text-grey-darker"> Mobile No. </th>
                                    <th class="text-left text-sm px-2 py-2 text-grey-darker" width="6%"> Actions </th>
                                </tr>
                            </thead>
                            <tbody>
                                @if(count($getParticipant)>0)
                                    ...
                                    @foreach($getParticipant as $info)
                                        <tr class="border-b">
                                            <td>{{$i++}}</td>
                                            <td>
                                                <a href="{{ url('/admin/student/show/'.$info->usersInfo->name) }}"> {{ $info->usersInfo->FullName }}</a>
                                            </td>
                                            <td>
                                                @if($info->usersInfo->email != null)
                                                    {{ $info->usersInfo->email }}
                                                @else
                                                    --
                                                @endif
                                            </td>
                                            <td>{{ $info->usersInfo->mobile_no }}</td>
                                            <td>
                                                @if( $conference->user_id == Auth::id() )
                                                    <a href="javascript:void(0);" class="btn bg-red-600 text-white rounded px-3 py-1 delete" data-id="{{$info->id}}">Delete</a>
                                                @else 
                                                    - 
                                                @endif
                                            </td>
        ...
        $(function(){
            $(document).on('click', '.delete', function(){
                ...
                if (r == true) {
                    ...
                    window.location.href = '{{url("admin/video-conference/remove-users")}}/'+getid;
                }
            });
        });
    </script>
