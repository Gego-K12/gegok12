<div class="relative">
    ...
    <div class="flex flex-wrap items-center justify-between my-3">
        <h1 class="admin-h1 mb-3 font-bold font-exo">Non-Teaching Staff ( {{ $totalStaffCount }} )</h1>
    ...
    @if (count($selected) > 0)
        <div class="bg-amber-50 border border-amber-200 rounded p-3 my-3 flex items-center justify-between flex-wrap gap-2">
            <div class="text-sm text-amber-800">
                <span>{{ count($selected) }} of {{ $staff->total() }} staff selected.</span>
                @if (count($selected) < $staff->total())
                    <button type="button" wire:click="selectAllMatching" class="text-amber-600 font-semibold text-sm ml-2">Select all {{ $staff->total() }} staff</button>
                @endif
            ...
            <table class="w-full">
                ...
                <tbody>
                    @forelse ($staff as $staffMember)
                        @php
                            $profile = $staffMember->userprofile;
                            $teacherProfile = $staffMember->latestTeacherProfile;
                        @endphp
                        <tr class="border-b" wire:key="staff-{{ $staffMember->id }}">
                            <td class="py-3 px-2 align-top">
                                <input type="checkbox" wire:model.live="selected" value="{{ $staffMember->id }}">
                            </td>
                            <td class="py-3 px-2">
                                <div class="flex items-center">
                                    <img src="{{ optional($profile)->AvatarPath }}" class="w-10 h-10 rounded-full mr-2">
                                    <div>
                                        <a href="{{ url('/admin/staff/show/'.$staffMember->name) }}" class="font-semibold text-sm text-blue-700 hover:underline">
                                            {{ optional($profile)->gender === 'female' ? 'Ms.' : 'Mr.' }} {{ $staffMember->FullName }}
                                        </a>
                            ...
                            <td class="py-3 px-2 align-top">
                                <span class="rounded-full px-2 py-1 text-xs font-semibold {{ $staffMember->status === 'exit' ? 'bg-red-100 text-red-700' : ($staffMember->status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-600') }}">
                                    {{ $staffMember->status === 'exit' ? 'Relieved' : ($staffMember->status === 'active' ? 'Active' : 'Inactive') }}
                                </span>
                            </td>
                            <td class="py-3 px-2 align-top">
                                @if ($staffMember->lastLogin)
                                    <span class="text-gray-700 text-sm">{{ $staffMember->lastLogin->created_at->format('d M Y, h:i A') }}</span>
                                @else
                    ...
                    @empty
                        <tr class="border-b">
                            <td colspan="{{ $dateOfBirthMonth ? 6 : 5 }}">
                ...
                <div class="modal-container w-full max-w-2xl px-8 mx-auto" style="margin: 0 auto; padding: 20px 30px; background-color: #fff; border-radius: 2px; box-shadow: 0 2px 8px rgba(0,0,0,.33);">
                    <div class="modal-header flex justify-between items-center mb-6">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-600">To {{ count($selected) }} selected staff</p>
                            </div>
                        </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea wire:model="message" placeholder="Type your message to the staff..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500" rows="8"></textarea>
