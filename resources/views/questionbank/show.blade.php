...
@section('content')
      ...
      <div>
		<h1 class="admin-h1 my-3 flex items-center">
			<a href="{{url('subject/chapters')}}" title="Back" class="rounded-full bg-gray-100 p-2">
    ...
    <div class="bg-white shadow my-5">
		<div class="flex border shadow py-3 px-5 text-white justify-between items-center">
			<div class="flex-col">
				<p class="text-gray-700 text-2xl font-medium">{{ ucfirst($subject->name) }}</p>
			</div>
			<div class="flex cursor-pointer">
				...
				<import-question url="{{url('')}}" subject_id="{{$subject->id}}"></import-question>
			</div>
		</div>
	</div>
	<div class="bg-white shadow my-5">
      <show-subject url="{{ url('/') }}"  subject_id="{{$subject->id}}" mode="admin"></show-subject>
      <div id="subjectchapter"></div>
   </div>
@endsection
