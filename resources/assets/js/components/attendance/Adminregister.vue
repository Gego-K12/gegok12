<template>
  <div class="bg-white shadow px-4 py-3">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div class="flex items-center gap-3" v-if="standardlist && standardlist.length">
        <label for="standardLink_id" class="text-sm font-semibold text-gray-700">Select Class</label>
        <select
          id="standardLink_id"
          v-model="selectedStandardLinkId"
          @change="handleStandardChange"
          class="tw-form-control text-sm"
        >
          <option v-for="standard in standardlist" :key="standard.id" :value="standard.id">{{ standard.standard_section }}</option>
        </select>
      </div>
      <a
        v-if="selectedStandardLinkId"
        :href="'/' + mode + '/attendance/add?standardLink_id=' + selectedStandardLinkId"
...
export default {
  ...
  props: ['url', 'mode', 'academicYearStart', 'today', 'standardlink_id', 'standardlist'],
  data() {
    return {
      selectedStandardLinkId: this.standardlink_id ? Number(this.standardlink_id) : null,
  ...
  methods: {
    ...
    handleStandardChange() {
      ...
      if (this.visibleStart) {
        const d = new Date(this.visibleStart + 'T00:00:00');
        const month = d.getFullYear() + '-' + String(d.getMonth() + 1).padStart(2, '0');
        this.fetchMonthSummary(month);
      }
    },
    fetchRegister() {
      if (! this.selectedStandardLinkId) {
        return;
      }

      this.loading = true;

      axios.get('/' + this.mode + '/attendance/student/register/'+this.selectedStandardLinkId+'/'+this.selectedDate).then(response => {
    ...
    fetchMonthSummary(month) {
      ...
      axios.get('/' + this.mode + '/attendance/student/register/summary/' + this.selectedStandardLinkId + '/' + month).then(response => {
