<template>
<div class="px-3 overflow-x-scroll lg:overflow-x-auto md:overflow-x-auto py-3" v-bind:class="[this.product_tab==4?'block' :'hidden']">
  <div class="relative">
    ...
    <div class="flex flex-wrap lg:flex-row justify-between items-center">
      <div class="">
        <h1 class="admin-h1 my-3">Product Condition</h1>
      </div>
        ...
        <table class="w-full">
          <thead class="bg-grey-light">
            <tr class="border-b">
              ...
              <th class="text-left text-sm px-2 py-2 text-grey-darker" >Condition </th>
          ...
          <tbody v-if="this.productconditions != ''" >
            <tr class="border-b" v-for="(productcondition,k1) in productconditions" :key="k=k1+1">
              <td class="py-3 px-2" >
                <p class="font-semibold text-xs">{{k}}</p>
              </td>
              
              <td class="py-3 px-2" >
                <p class="font-semibold text-xs">{{productcondition.productcode.code}}</p>
              </td>
              <td class="py-3 px-2">
                <p class="font-semibold text-xs">{{productcondition.condition}}
                  
                </p>
              </td>
              <td class="py-3 px-2">
                <p class="font-semibold text-xs">{{productcondition.date}}
              ...
              <!--< td class="py-3 px-2">
                <div class="flex items-center">                
             <a @click="productconditionEdit(productcondition.id)" class="cursor-pointer" title="Edit">
    ...
    C443.754,73.631,440.467,81.575,434.603,87.419z"></path></g></g></svg>
                  ...
                  <a href="#" @click="deleteproductcondition(productcondition.id)" class="cursor-pointer" title="Delete">
...
export default {
  ...
  methods:{
    getlist(page=1)
    {
       axios.get('/admin/productcondition/'+this.productid+'/list?page='+this.page).then(response=>{
                  console.log(response);
                  this.productconditions=response.data.product_condition.data;
                  this.page_count = response.data.product_condition.last_page;
                  this.total = response.data.product_condition.total;
    ...
    deleteproductcondition(id) 
    {
      ...
      swal({
        ...
        if (isConfirm) 
        {
          axios.delete('/admin/productcondition/'+id+'/delete').then(response => {
