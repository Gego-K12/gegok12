            ...
            <div class="row justify-content-center">
                <div class="flex flex-wrap lg:flex-row items-center">
                    <div class="flex items-center justify-between w-full">
                        <h1 class="admin-h1 my-3">Purchase List</h1>
                        <div class="flex justify-end">
                           <!--  <a :href="'/admin/sales/add'" class="no-underline text-white px-4 my-3 mx-1 flex items-center custom-green py-1 justify-center cursor-pointer rounded">
                          ...
                          <!--   <div class="search relative w-48 mx-2 my-3">
                                ...
                                <a href="#" @click="searchSales()" class="no-underline text-white px-4 mx-1 py-1 absolute right-0 focus:outline-none">
                ...
                <div class="flex flex-wrap custom-table my-3 overflow-auto">
                    <table class="w-full">
                        <thead>
                            <tr class="bg-grey-light">
                                ...
                                <th class="text-left text-sm px-2 py-2 text-grey-darker">Price Per Item</th>
                                <th class="text-left text-sm px-2 py-2 text-grey-darker">Purchase Date</th>
                               
                                <th>Action</th>
                            </tr>
                        </thead>
                        <tbody v-if="this.purchases != ''">
                            <tr v-for="purchase in purchases">
                                <td><a :href="url+'/'+mode+'/purchase/'+purchase.id+'/show'">{{purchase.product_name}}</a></td>
                                <td>{{purchase.quantity}}</td>
                                <td>{{purchase.price_per_item}}</td>
                                <td>{{purchase.purchased_date}}</td>
                             
                                <td>
                                    <div class="flex items-center">                
                                        <a :href="url+'/'+mode+'/purchase/'+purchase.id+'/edit'" class="cursor-pointer" title="Edit">
                                        ...
                                        <a :href="url+'/'+mode+'/purchase/'+purchase.id+'/show'" target="_blank" class="cursor-pointer" title="Show">
                                        ...
                                        <a @click="deletepurchase(purchase.id)" class="cursor-pointer" title="Delete">
                    ...
                    <paginate v-model="page" :page-count="this.page_count" :page-range="3" :margin-pages="1" :click-handler="purchaseshow" :prev-text="'<'" :next-text="'>'" :container-class="'pagination'" :page-class="'page-item'" :prev-link-class="'prev'" :next-link-class="'next'"></paginate>
                </div>
            </div>
        </div>
    </div>
...
<script>
    export default {
        ...
        methods:
        {
            purchaseshow(page=1)
            {
                axios.get(this.url+'/'+this.mode+'/purchase/list?page='+this.page+'&'+this.searchquery).then(response=>{
            ...
            deletepurchase(id) 
            {
                ...
                swal({
                    ...
                    if (isConfirm) 
                    {
                        axios.delete(thisswal.url+'/'+thisswal.mode+'/purchase/'+id+'/delete').then(response => {
            ...
            searchSales()
            {
                ...
                this.final=this.url+'/'+this.mode+'/purchase/show?search='+this.searchquery;
            ...
            resetForm()
            {
                window.location.href=this.url+'/'+this.mode+'/purchase/show';
            }
        },
