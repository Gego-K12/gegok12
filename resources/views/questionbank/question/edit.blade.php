...
@section('content')
   <div class="relative">
      <div class="flex flex-wrap lg:flex-row justify-between">
         <div>
            <h1 class="admin-h1 my-3 flex items-center">
               <a href="{{ url('/chapter/show/'.$question->chapter_id) }}" title="Back" class="rounded-full bg-gray-100 p-2">
               ...
               <span class="mx-3">Edit Question</span>
            </h1>
         </div>
      </div>
      ...
      <div class="flex border shadow py-3 px-5 text-white justify-between items-center">
         <div class="flex-col">
            <p class="text-gray-700 text-2xl font-medium">{{ ucfirst($question->chapter->name) }}</p>
         </div>
         <div class="flex">
            <div class="flex-col leading-relaxed">
               <p class="text-gray-700 font-medium">Subject name:<span class="font-semibold">{{ $question->chapter->subject->name }} </span></p>
               <!-- <p class="text-gray-700 font-medium">Top scorer:<span class="font-semibold"> </span></p> -->
            </div>
         </div>
      </div>
   </div>
      @include('partials.message')
      <edit-test-question url="{{ url('/') }}" question_id="{{$question->id}}" subject_id="{{$question->chapter->subject_id}}" ></edit-test-question>
   </div>
@endsection