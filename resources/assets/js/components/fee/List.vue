<template>
    <div class="">
        <Teleport to="#add_fees">
            <div class="flex flex-wrap lg:flex-row justify-between">
                <div class="">
                    <h1 class="admin-h1 my-3">Fees</h1>
                </div>
                <div class="relative flex items-center w-8/4 lg:w-1/3 md:w-1/4 justify-end">
                    <div class="flex items-center w-full justify-end">
                        ...
                        <a :href="url+'/'+mode+'/fee/add'" class="no-underline text-white px-4 my-3 mx-1 flex items-center custom-green py-1 justify-center">
            ...
            <table class="w-full">
                ...
                <tbody class="bg-grey-light" v-if="Object.keys(this.feelist).length > 0">
                    <tr class="border-b" v-for="fee in feelist">
                        <td class="py-3 px-2">
                            <p v-if="fee.fee_type != null">{{ fee.fee_type }}</p>
                            <p v-else>--</p>
                        </td>
                        <td class="py-3 px-2">
                        ...
                        <td class="py-3 px-2">  
                            <a :href="url+'/admin/feedetail/show/'+fee.id" class="" title="Show">{{ fee.name }}</a>
                        </td>
                        <td class="py-3 px-2">{{ fee.term }}</td>
                        <td class="px-3 py-2">{{ fee.amount }}</td>
                        <td class="py-3 px-2">{{ fee.start_date }}</td>
                        <td class="py-3 px-2">{{ fee.end_date }}</td>
                        <!-- <td class="py-3 px-2 cursor-pointer" @click="showPaidStudent([fee.id])">{{ paidCount[fee.id] }}</td> -->
                        <td class="py-3 px-2">{{ fee.paid_count }}</td>
                        <!-- <td class="py-3 px-2 cursor-pointer" @click="showUnpaidStudent([fee.id])">{{ unpaidCount[fee.id] }}</td> -->
                        ...
                        <td class="py-3 px-2">
                            <div class="flex items-center">
                                <a :href="url+'/'+mode+'/fee/edit/'+fee.id" class="" title="Edit">
                                ...
                                <a :href="url+'/'+mode+'/feedetail/show/'+fee.id" class="" title="Show">
                                ...
                                <a href="#" @click="deleteFee(fee.id)" class="delete" title="Delete" v-if="fee.paid_count == 0">
                                    ...
                                    <tr class="border-b" v-for="student in paidStudents[fee['id']]">
                                        <td class="py-3 px-2 cursor-pointer">
                                            <p class="font-semibold text-xs">
                                                <a :href="url+'/admin/student/show/'+student.user_name" target="_blank">{{ student.user_fullname }}</a>
                                            </p>
                                        </td>
                                        <td class="py-3 px-2">
                            ...
                            <table class="w-full">
                                ...
                                <tbody>
                                    <tr class="border-b" v-for="student in unpaidStudents[fee['id']]">
                                        <td class="py-3 px-2 cursor-pointer">
                                            <p class="font-semibold text-xs">
                                                <a :href="url+'/admin/student/show/'+student.name" target="_blank">{{ student.fullname }}</a>
                                            </p>
                                        </td>
                                    </tr>
                                </tbody>
                                <tbody v-if="unpaidStudents == ''">
                                    <tr class="border-b">
                                        <td colspan="1">
                                            <p class="font-semibold text-s" style="text-align: center">No Records Found</p>
                                        </td>
...
<script>
    ...
    export default {
        ...
        methods:
        {
            getData()
            {
                axios.get('/'+this.mode+'/fees/list/'+this.status+'?standardLink_id='+this.standardLink_id+'&search='+this.search).then(response => {
            ...
            {
                axios.get('/'+this.mode+'/fees/list').then(response => {
            ...
            deleteFee(id) 
            {
                ...
                swal({
                    ...
                    if (isConfirm) 
                    {
                        axios.get(thisswal.url+ '/'+thisswal.mode+'/fee/delete/'+ id).then(response => {
