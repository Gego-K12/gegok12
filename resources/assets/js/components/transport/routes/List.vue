    ...
    <div class="flex flex-wrap lg:flex-row justify-between items-center">
      ...
      <div class="relative flex items-center w-8/12 lg:w-1/4 md:w-1/4 justify-end">
        <div class="flex items-center">
          <a :href="url+'/admin/transport/route/create'" class="no-underline text-white px-4 my-3 mx-1 flex items-center custom-green py-1 justify-center cursor-pointer">
    ...
    <div class="">
      <div class="flex flex-wrap custom-table my-3 overflow-auto">
        <table class="w-full">
          <thead class="bg-grey-light">
            <tr class="border-b">
              <th class="text-left text-sm px-2 py-2 text-grey-darker">Route</th>
               ...
               <th class="text-left text-sm px-2 py-2 text-grey-darker" width="35%">
               <div class="flex items-center justify-between">
                 <p class="w-1/2"> Stop </p>
               ...
               <!-- Subjects & Teacher -->
               <th class="text-left text-sm px-2 py-2 text-grey-darker text-center" width="10%">Status</th>
          ...
          <tbody v-if="this.routes != ''">
            <tr class="border-b" v-for="(route,k1) in routes" :key="k=k1+1" >
              <td class="py-3 px-2" >
                  <p class="font-semibold text-xs">{{route.name}}</p>
              </td>
                <td class="px-0 py-0" ><!-- width="23%" -->
                       
                          <table class="w-full subject-table" style="border:unset;">
                               <tr v-for="stop in route.stoppages">
                                <td width="30%" style="border: unset;border-right:1px solid #cbd5e0;"> {{ stop.stopname }}  </td>
                                 <td width="30%" style="border: unset;border-right:1px solid #cbd5e0;"> {{ stop.pickup_time }}  </td>
                                 <td width="30%" style="border: unset;"> {{ stop.drop_time }}  </td>
                              </tr>
                           </table>
                     </td>
             <!--  <td class="py-3 px-2 flex" >
                  <p class="font-semibold text-xs">{{route.vehicle_count}}
                  </p>
              </td> -->
             <td class="py-3 px-2" >
              <p v-if="route.status==0" class="font-semibold text-xs">Inactive</p>
                 <p v-else="" class="font-semibold text-xs">Active</p>
               </td>
              <td class="py-3 px-2">
                <div class="flex items-center">                
                  <a :href="url+'/admin/transport/route/'+route.id+'/edit'" class="cursor-pointer" title="Edit">
                  ...
                  <a  :href="url+'/admin/transport/route/'+route.id+'/show'" class="cursor-pointer">
                  ...
                  <a href="#" @click="deletetopic(route.id)" class="cursor-pointer" title="Delete">
       ...
       <table class="w-full border" >
          <thead class="bg-grey-light">
            <tr class="border-b">
              ...
              <th class="text-left text-xs px-2 py-2 text-grey-darker">Formula </th>
            </tr>
          </thead>   
          <tbody v-if="this.payrollitems != ''" >
           ...
           <!--  <div class="flex justify-between items-center py-1">
               <p class="font-semibold text-xs">{{route.created_at}}</p>
  ...
  methods:
  {
    ...
    deletetopic(id) 
    {
      ...
      swal({
        ...
        if (isConfirm) 
        {
          axios.delete(thisswal.url+'/admin/transport/route/'+id+'/delete').then(response => {
    ...
    detail(id)
      {
        ...
        this.show=1;
        axios.get(this.url+'/accountant/payroll/route/'+id+'/editshow').then(response => {
