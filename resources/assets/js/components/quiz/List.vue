                ...
                <table class="w-full">
                    <thead class="bg-grey-light">
                        <tr class="border-b">
                            ...
                            <th class="text-left text-sm px-2 py-2 text-grey-darker">Topic</th>
                            <th class="text-left text-sm px-2 py-2 text-grey-darker">Timer(sec)</th>
                            <th class="text-left text-sm px-2 py-2 text-grey-darker">No.of questions</th>
                            <th class="text-left text-sm px-2 py-2 text-grey-darker">No.of Participants</th>
                            <th class="text-left text-sm px-2 py-2 text-grey-darker">Top Score</th>
                            <th class="text-left text-sm px-2 py-2 text-grey-darker">Top Scorer</th>
                            <th class="text-left text-sm px-2 py-2 text-grey-darker">Average</th>
                            <th class="text-left text-sm px-2 py-2 text-grey-darker">Actions</th>
                        </tr>
                    </thead>   
                    <tbody v-if="Object.keys(this.topics).length > 0">
                        <tr class="border-b" v-for="(topic,k1) in topics" :key="k=k1+1">
                            <td class="py-3 px-2">
                                <p class="font-semibold text-xs">{{ k }}</p>
                            </td>
                            <td class="py-3 px-2">
                                <p class="font-semibold text-xs">{{ topic.name }}</p>
                            </td>
                            <td class="py-3 px-2">
                                <p v-if="topic.timer!=null" class="font-semibold text-xs">{{topic.timer}}</p>
                                <p v-else="" class="font-semibold text-xs">Not set</p>
                            </td>
                            <td class="py-3 px-2">
                                <p class="font-semibold text-xs">{{ topic.questions }}</p>
                            </td>
                            <td class="py-3 px-2">
                                <p class="font-semibold text-xs">{{ topic.participants }}</p>
                            </td>
                            <td class="py-3 px-2">
                                <p class="font-semibold text-xs">{{ topic.topscore }}</p>
                            </td>
                            <td class="py-3 px-2">
                                <p class="font-semibold text-xs">{{ topic.topscorer }}</p>
                            </td>
                            <td class="py-3 px-2">
                                <p class="font-semibold text-xs">{{ topic.avgscore }}</p>
                            </td>
                            <td class="py-3 px-2">
                                <div class="flex items-center">                
                                    <a href="#" @click="topicEdit(topic.id)" class="cursor-pointer" title="Edit">
                                    ...
                                    <a :href="url+'/teacher/quiz/'+topic.id+'/show'" title="Show">
                                    ...
                                    <a href="#" @click="deletetopic(topic.id)" title="Delete">
                    ...
                    <div class="modal-body">
                        <div class="flex flex-col">
                            <div class="w-full lg:w-1/4"> 
                                <label for="topic" class="tw-form-label">Topic<span class="text-red-500">*</span></label>
                            </div>
                            <div class="my-2 w-full">
                                <input type="text" name="topic" v-model="topic" class="tw-form-control w-full"><br>
                                <span v-if="errors.topic" class="text-red-500 text-xs font-semibold">{{ errors.topic[0] }}</span>
                            </div>
                        </div>
                        <div class="flex flex-col">
                            <div class="w-full flex"> 
                                <label for="select" class="tw-form-label">Timer</label>
                            </div>
                            <div class="my-2">
                                <input type="checkbox" v-model="select" v-bind:true-value="1" v-bind:false-value="0" @click="change()">
                            </div>
                        </div>
                        <div v-show="timershow" class="flex flex-col">
                            <div class="w-full flex"> 
                                <label for="timer" class="tw-form-label">Question time in sec</label>
                            </div>
                            <div class="my-2 w-full">
                                <input type="text" name="timer" v-model="timer" class="tw-form-control w-full"><br>
                                <span v-if="errors.timer" class="text-red-500 text-xs font-semibold">{{ errors.timer[0] }}</span>
                            </div>
                        </div>
                        <div class="flex items-center">
                            <div class="my-2 w-full lg:w-3/4">
                                <a href="#" class="btn btn-submit blue-bg text-white rounded px-3 py-1 cursor-pointer text-sm font-medium" @click="addtopic()">Submit</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-wrapper px-4">
                <div class="modal-container w-full  max-w-md px-8 mx-auto">
                    ...
                    <div class="modal-body">
                        <div class="flex flex-col">
                            <div class="w-full lg:w-1/4"> 
                                <label for="edittopic" class="tw-form-label">Topic<span class="text-red-500">*</span></label>
                            </div>
                            <div class="my-2 w-full">
                                <input type="text" name="edittopic" v-model="edittopic" class="tw-form-control w-full"><br>
                                <span v-if="errors1.topic" class="text-red-500 text-xs font-semibold">{{ errors1.topic[0] }}</span>
                            </div>
                        </div>
                    </div>
                    <div class="flex flex-col">
                    ...
                    <div class="flex flex-col" v-if="this.select == 1">
                        <div class="w-full flex"> 
                            <label for="obtained_marks" class="tw-form-label">Question time in sec</label>
                        </div>
                        <div class="my-2 w-full">
                            <input type="text" name="edittimer" v-model="edittimer" class="tw-form-control w-full"><br>
                            <span v-if="errors1.timer" class="text-red-500 text-xs font-semibold">{{ errors1.timer[0] }}</span>
                        </div>
                    </div>
                    <div class="flex items-center">
                        <div class="my-2 w-full lg:w-3/4">
                            <a href="#" @click="updatetopic(edit.id)" class="btn btn-submit blue-bg text-white rounded px-3 py-1 cursor-pointer text-sm font-medium">Submit</a>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        methods:
        {
            getlist()
            {
                axios.get('/teacher/quiz/list?search='+this.search).then(response => {
            ...
            addtopic()
            {
                axios.post(this.url+'/teacher/quiz/add',{
                    topic:this.topic,
                    timer:this.timer,
                    select:this.select,
                }).then(response => {
            ...
            {
                this.editshow=1;
                axios.get(this.url+'/teacher/quiz/'+id+'/edit').then(response => {
                    this.edit=response.data;
                    this.edittopic=this.edit.name;
                    this.edittimer=this.edit.timer;
                    if(this.edit.timer != null)
                    {
                        this.select = 1;
                        this.timershow=true;
                        this.edittimer=this.edit.timer;
            ...
            updatetopic(id)
            {
                axios.post(this.url+'/teacher/quiz/'+id+'/update',{
                    topic:this.edittopic,
                    timer:this.edittimer,
                    select:this.select,
                }).then(response => {
                    this.success = response.data.message;
                    this.close();
            ...
            deletetopic(id) 
            {
                ...
                swal({
                    ...
                    if (isConfirm) 
                    {
                        axios.delete(thisswal.url+'/teacher/quiz/'+id+'/delete').then(response => {
