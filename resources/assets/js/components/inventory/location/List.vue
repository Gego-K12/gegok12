<template>
  <div class="relative">
    ...
    <div class="flex flex-wrap lg:flex-row justify-between items-center">
      <div class="">
        <h1 class="admin-h1 my-3">Location</h1>
      </div>
   ...
   <span class="my-2 mr-2">Show</span>
    <span>
     <div class="relative">
        ...
        <div class="pointer-events-none absolute top-0 bottom-0 right-0 flex items-center px-1 text-grey-darker">
        ...
        <table class="w-full">
          <thead class="bg-grey-light">
            <tr class="border-b">
              ...
              <th class="text-left text-sm px-2 py-2 text-grey-darker">Location Name</th>
          ...
          <tbody v-if="this.locations != ''" >
            <tr class="border-b" v-for="(location,k1) in locations" :key="k=k1+1" >
              <td class="py-3 px-2" >
                <p class="font-semibold text-xs">{{k}}</p>
              </td>
              <td class="py-3 px-2" >
                <p class="font-semibold text-xs">{{location.name}}</p>
              </td>
              <td class="py-3 px-2">
                <p v-if="location.status==0" class="font-semibold text-xs">Inactive</p>
                 <p v-else="" class="font-semibold text-xs">Active</p>
              </td>
              <td class="py-3 px-2">
                <div class="flex items-center">                
                  <a @click="locationEdit(location.id)" class="cursor-pointer" title="Edit">
                  ...
                  <a href="#" @click="deletelocation(location.id)" class="cursor-pointer" title="Delete">
          ...
          <div class="modal-body" >
            <div class="flex items-center">
              ...
              <div class="my-2 w-full">
                <input type="text"  name="name" v-model="name" class="tw-form-control w-full">
            ...
            <div class="flex items-center">
              ...
              <div class="my-2 w-full lg:w-3/4 ">
                <input type="button" name=""  value="submit" @click="addlocation()" class="btn btn-submit blue-bg text-white rounded px-3 py-1 my-2 text-sm font-medium cursor-pointer">
              </div>
            </div>
          </div>
        ...
        <div class="modal-container w-full  max-w-md px-8 mx-auto">
          ...
          <div class="modal-body" >
            <div class="">
              ...
              <div class="my-2 w-full">
                <input type="text"  name="editname" v-model="editname" class="tw-form-control w-full">
            ...
            <div class="my-2 w-full lg:w-3/4 flex items-center">
            <input type="checkbox" v-model="editstatus" v-bind:true-value="1" v-bind:false-value="0" name="status"  ><span class="mx-2 text-sm">Active</span>
  </div>
        </div></div>
            ...
            <div class="flex items-center">
              ...
              <div class="my-2 w-full lg:w-3/4 ">
                <input type="button" name=""  value="submit" @click="updatelocation(edit.id)" class="btn btn-submit blue-bg text-white rounded px-3 py-1 my-2 text-sm font-medium cursor-pointer">
              </div>
            </div>
          </div>
...
export default {
 // props:['url','categoryid'],
  ...
  methods:{
    getlist(page=1)
    {
       axios.get('/admin/location/list?page='+this.page+'&perpage='+this.per_page+'&search='+this.searchresult).then(response=>{
    ...
    {
     this.editshow=1;
      axios.get('/admin/location/'+id+'/edit').then(response => {
    ...
    {
     // alert(id);
      axios.post('/admin/location/'+id+'/update',{
        name:this.editname,
    ...
    deletelocation(id) 
    {
      ...
      swal({
        ...
        if (isConfirm) 
        {
          axios.delete('/admin/location/'+id+'/delete').then(response => {
