...
@section('content')
      ...
      <div>
		<h1 class="admin-h1 my-3 flex items-center">
			<a href="{{url('subject/chapter/'.$chapter->subject_id.'/show')}}" title="Back" class="rounded-full bg-gray-100 p-2">
			...
			<span class="mx-3">Chapter</span>
		</h1>
	</div>
    <div class="bg-white shadow my-5">
		<div class="flex border shadow py-3 px-5 text-white justify-between items-center">
			<div class="flex-col">
				<p class="text-gray-700 text-2xl font-medium">{{ ucfirst($chapter->name) }}</p>
			</div>
			<div class="flex">
				<div class="flex-col leading-relaxed">
					<p class="text-gray-700 font-medium">Subject name:<span class="font-semibold">{{ $chapter->subject->name }} </span></p>
					<p class="text-gray-700 font-medium">Total Questions:<span class="font-semibold">{{ $chapter->questions->count() }} </span></p>
				</div>
			</div>
		</div>
	</div>
	<div class="bg-white shadow my-5">
      <show-chapter url="{{ url('/') }}" chapter_id="{{$chapter->id}}" subject_id="{{$chapter->subject_id}}" mode="admin"></show-chapter>
      <div id="questionchapter"></div>
   </div>
@endsection
