...
@section('content') 
    <div class="w-full">
        <h1 class="admin-h1 my-3 flex items-center">
            <a href="{{ url('/alumni/show/'.\Auth::user()->name) }}" title="Back" class="rounded-full bg-gray-100 p-2">
            ...
            <span class="mx-3">Edit Form</span>
        </h1>
        @include('partials.message')
        <form method="POST" action="{{ url('/alumni/edit') }}" enctype="multipart/form-data">
            @csrf
            <add-alumni url="{{ url('/') }}" type="edit"></add-alumni>
            <div id="edit_alumniform"></div>
        </form>
    </div>
@endsection