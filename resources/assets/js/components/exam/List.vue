<template>
    <div>
        <vue-good-table mode="remote" :columns="columns" :rows="rows" :group-options="{ enabled: true , headerPosition: 'top' }" :search-options="{ enabled: false }"> 
            ...
            <template #table-header-row="props">
                <div v-if="props.row.action==''" class="w-full flex justify-between">
                    ...
                    <div class="flex" v-if="props.row['children']==''">
                        <a class="block btn px-2 py-1 mx-2 bg-blue-600 text-white rounded text-sm" :href=props.row.hallticketurl>Hall ticket</a>
                    ...
                    <div class="w-32 relative" v-if="props.row['children']!=''">
                        ...
                        <a class="text-sm rounded px-2 py-1 flex items-center justify-between btn btn-primary submit-btn w-full"  v-show="props.row['exam_status']!='pending'" @click="showeventlink(props.row.id)" id="show" >
...
<script>
    ...
    export default {
        ...
        methods: 
        {
            getData(page=1)
            {
                axios.get('/admin/exam/show?'+this.query + '&page=' + this.page).then(response => {
