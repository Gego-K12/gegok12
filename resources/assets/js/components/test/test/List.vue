  ...
  <div class="overflow-x-auto" >
    <div class="relative">
      ...
      <div class="flex flex-wrap lg:flex-row justify-between items-center my-1">
        <div class="">
          <h1 class="admin-h1">Test Paper</h1>
        </div>
        <div class="relative flex items-center w-8/12 lg:w-1/4 md:w-1/4 justify-end">
          <div class="flex items-center"> 
           <a @click="showAdd()" class="no-underline text-white px-4 my-3 mx-1 flex items-center cursor-pointer custom-green py-1 justify-center">
          ...
          <table class="w-full">
            <thead class="bg-grey-light">
              <tr class="border-b">
                ...
                <th class="text-left text-sm px-2 py-2 text-grey-darker">Test At</th>
    ...
    <!--show modal  -->
     <div v-if="this.add_show == 1" class="modal modal-mask">
      <div class="modal-wrapper px-4">
        <div class="modal-container w-full  max-w-md px-8 mx-auto">
          <div class="modal-header flex justify-between items-center">
            <h2  v-show="this.status=='add'" >Add Test</h2>
             <h2 v-show="this.status=='edit'" >Edit Test</h2>
            <button id="close-button" class="modal-default-button text-2xl py-1" @click="closeAdd()">
              &times;
            </button>
          </div>
          <div class="modal-body" >
            <div class="flex flex-col">
                            <div class="w-full lg:w-1/4"> 
                                <label for="chapter_name" class="tw-form-label">Class<span class="text-red-500">*</span></label>
                            </div>
               ...
               <div class="flex flex-col">
                            <div class="w-full lg:w-1/4"> 
                                <label for="chapter_name" class="tw-form-label">Subject<span class="text-red-500">*</span></label>
                            </div>
                            <div class="my-2 w-full">
               ...
               <div class="flex flex-col">
                            <div class="w-full lg:w-1/4"> 
                                <label for="total_marks" class="tw-form-label">Total Marks<span class="text-red-500">*</span></label>
                            </div>
                            <div class="my-2 w-full">
                                <input type="text" name="total_marks" v-model="total_marks" class="tw-form-control w-full"><br>
                                <span v-if="errors.total_marks" class="text-red-500 text-xs font-semibold">{{ errors.total_marks[0] }}</span>
                            </div>
              </div>
              <div class="flex flex-col">
                            <div class="w-full lg:w-1/4"> 
                                <label for="test_date" class="tw-form-label">Test Date<span class="text-red-500">*</span></label>
                            </div>
                            <div class="my-2 w-full">
                               <VueDatePicker
                                 v-model="test_date"
                               ...
                               />
                                <span v-if="errors.test_date" class="text-red-500 text-xs font-semibold">{{ errors.test_date[0] }}</span>
                            </div>
              </div>
              <div class="flex flex-col">
                            <div class="w-full lg:w-1/4"> 
                                <label for="duration" class="tw-form-label">Duration<span class="text-red-500">*</span></label>
                            </div>
                            <div class="my-2 w-full">
                                <input type="text" name="duration" v-model="duration" class="tw-form-control w-full"><br>
                                <span v-if="errors.duration" class="text-red-500 text-xs font-semibold">{{ errors.duration[0] }}</span>
                            </div>
              </div>
               <div class="flex items-center">
                            <div v-show="this.status=='add'" class="my-2 w-full lg:w-3/4">
                                <a href="#" class="btn btn-submit blue-bg text-white rounded px-3 py-1 cursor-pointer text-sm font-medium" @click="addTest()">Submit</a>
                            </div>
                             <div v-show="this.status=='edit'" class="my-2 w-full lg:w-3/4">
                                <a href="#" class="btn btn-submit blue-bg text-white rounded px-3 py-1 cursor-pointer text-sm font-medium" @click="updateTest()">Update</a>
                            </div>
                </div>
          </div>
...
import '@vuepic/vue-datepicker/dist/main.css'
  export default {
    ...
    methods:
    {
      getlist()
      {
        axios.get('/test/paper/list?page='+this.page).then(response => {
    ...
    deleteTest(id) 
    {
      ...
      swal({
        ...
        if (isConfirm) 
        {
          axios.get(thisswal.url+'/test/'+id+'/delete').then(response => {
    ...
    reset()
    {
     ...
     addTest()
      {
        axios.post(this.url+'/test/create',{
                    name:this.name,
    ...
    editList()
    {
      axios.get('/test/'+this.test_id+'/edit').then(response => {
          this.edittest    = response.data.data; 
          this.name=this.edittest.name;
          this.total_marks=this.edittest.total_marks;
          this.standardLink_id=this.edittest.standardLink_id;
          this.subject_id=this.edittest.subject_id;
          this.test_date=this.edittest.test_at;
          this.duration=this.edittest.duration;       
        });
    ...
    updateTest()
      {
        axios.post(this.url+'/test/'+this.test_id+'/update',{
                    name:this.name,
                    total_marks:this.total_marks,
                    standardLink_id:this.standardLink_id,
                    subject_id:this.subject_id,
