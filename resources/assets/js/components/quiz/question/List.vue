<template>
  <div class="relative">
    ...
    <div class="flex flex-wrap lg:flex-row justify-between items-center">
      <div class="">
        <h1 class="admin-h1 my-3">Questions</h1>
      </div>
      <div class="relative flex items-center w-8/12 lg:w-1/4 md:w-1/4 justify-end">
        <div class="flex items-center">
          ...
          <a :href="url+'/teacher/quiz/question/add'" target="_blank" class="no-underline text-white px-4 my-3 mx-1 flex items-center custom-green py-1 justify-center">
      ...
      <div class="flex flex-wrap custom-table my-3 overflow-auto">
        <table class="w-full">
          ...
          <tbody v-if="this.questions != ''"  >
            <tr class="border-b" v-for="question,index in questions"  >
              <td class="py-3 px-2" >
                <p class="font-semibold text-xs">{{index+1}}</p>
              </td>
              <td class="py-3 px-2" >
                <p class="font-semibold text-xs">{{question.question}}</p>
              </td>
              <td class="py-3 px-2" >
                <p class="font-semibold text-xs">{{question.questiontype.name}}</p>
              </td>
               <td class="py-3 px-2" >
                <p class="font-semibold text-xs">{{question.quiztopic.name}}</p>
              </td>
          ...
          <div class="modal-body" >
            <div class="flex items-center bg-blue-300">
              <div class="w-full lg:w-1/4"> 
                <label for="obtained_marks" class="tw-form-label">Question</label>
              </div>
              <div class="my-2 w-full lg:w-3/4 ">
                <input type="text"  name="" v-model="topic"><br>
                <span  class="text-red-600"><small class="text-red=600"></small></span>
              </div>
            </div>
            <div class="flex items-center">
...
export default {
  ...
  methods:{
    getlist()
    {
      axios.get(this.url+'/teacher/quiz/'+this.topic_id+'/questions').then(response => {
    ...
    addtopic()
    {
     ...
     axios.post(this.url+'/teacher/quiz/add',{topic:this.topic,}).then(response => {
    ...
    updatetopic(id)
    {
      axios.post(this.url+'/teacher/quiz/'+id+'/update',{topic:this.edittopic}).then(response => {
        this.success    = response.data.message;
        this.close();
        this.getlist();
      }).catch(error=>{
    ...
    deletetopic(id) 
    {
      ...
      swal({
        ...
        if (isConfirm) 
        {
          axios.get(thisswal.url+'/teacher/quiz/'+id+'/delete').then(response => {
