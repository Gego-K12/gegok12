...
class InstallTransportModule extends Command
{
    protected $signature = 'module:install-transport';
    ...
    public function handle()
    {
        try{
           ...
           // $repo = $this->ask('Enter the repository URL');
            ...
            $repo='https://github.com/gego-k12/transport.git';
            $package='gegok12/transport:v1.0.x-dev';
           ...
           /* if (!$repo || !$package || !$username || !$token) {
            ...
            foreach (['transport-routes', 'transport-components', 'transport-views', 'transport-migrations', 'transport-config', 'transportapi-routes'] as $tag) {
            ...
            // $customRoutes = [
            //     "if (file_exists(base_path('routes/gtransport.php'))) {\n    require base_path('routes/gtransport.php');\n}",
            //     "if (file_exists(base_path('routes/gtransportapi.php'))) {\n    require base_path('routes/gtransportapi.php');\n}"
            // ];
            ...
            $customRoutes = [
                "routes/web.php"=>"if (file_exists(base_path('routes/gtransport.php'))) {\n    require base_path('routes/gtransport.php');\n}",
                "routes/api.php"=>"if (file_exists(base_path('routes/gtransportapi.php'))) {\n    require base_path('routes/gtransportapi.php');\n}"
            ];
            ...
            $importLine = "import { registerTransport } from './gtransport'";
            ...
            if (!str_contains($content, "registerTransport(app)")) {
            ...
            if ($this->confirm('Do you want to run seeders?', true)) {
                ...
                exec(
                    'php artisan db:seed --class="Gegok12\\Transport\\Database\\Seeders\\TransportDatabaseSeeder" --force 2>&1',
                    $output,
                    $status
                );
