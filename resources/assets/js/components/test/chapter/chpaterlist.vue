  ...
  <div class="px-3 overflow-x-scroll lg:overflow-x-auto md:overflow-x-auto py-3" v-bind:class="[this.quiz_tab==2?'block' :'hidden']">
    <div class="relative">
      ...
      <div class="flex flex-wrap lg:flex-row justify-between items-center my-1">
        <div class="">
          <h1 class="admin-h1">Chapters</h1>
        </div>
        <div class="relative flex items-center w-8/12 lg:w-1/4 md:w-1/4 justify-end">
          <div class="flex items-center"> 
            <a :href="url+'/chapter/create'"  class="no-underline text-white px-4 mx-1 flex items-center custom-green py-1 justify-center">
          ...
          <table class="w-full">
            <thead class="bg-grey-light">
              <tr class="border-b">
                ...
                <th class="text-left text-sm px-2 py-2 text-grey-darker"> Question Count</th>
                <th class="text-left text-sm px-2 py-2 text-grey-darker" width="10%"> Actions </th>
              </tr>
            </thead>   
            <tbody v-if="this.chapters != ''">
              <tr class="border-b" v-for="(chapter,k1) in chapters" :key="k=k1+1"  >
                <td class="py-3 px-2">
                  <p class="font-semibold text-xs">{{k}}</p>
                </td>
                <td class="py-3 px-2">
                  <p class="font-semibold text-xs">{{chapter.name}}</p>
                </td>
                 <td class="py-3 px-2">
                  <p class="font-semibold text-xs">{{chapter.question_count}}</p>
                </td>
                <td class="py-3 px-2">
                  <div class="flex items-center">
                  <a :href="url+'/chapter/show/'+chapter.id" class="cursor-pointer">
                    ...
                    <a @click="detail(chapter.id,chapter.name)" target="_blank" title="Edit">
                    ...
                    <a @click="deletechapter(chapter.id)" title="Delete">
          ...
          <div class="modal-body" >
            <div class="flex flex-col">
                            ...
                            <div class="my-2 w-full">
                                ...
                                <span v-if="errors.chapter_name" class="text-red-500 text-xs font-semibold">{{ errors.chapter_name[0] }}</span>
                            </div>
              </div>
               <div class="flex items-center">
...
<script>
  ...
  export default {
    ...
    methods:
    {
      getlist()
      {
        axios.get('/subject/chapter/'+this.subject_id+'/list?page='+this.page).then(response => {
    ...
    deletechapter(id) 
    {
      ...
      swal({
        ...
        if (isConfirm) 
        {
          axios.get(thisswal.url+'/chapter/'+id+'/delete').then(response => {
