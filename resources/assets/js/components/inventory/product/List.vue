<template>
                    ...
                    <div class="flex items-center justify-between w-full">
                        ...
                        <div class="relative flex items-center w-8/4 lg:w-1/3 md:w-1/4 justify-end">
                            <div class="flex items-center w-full justify-end">
                                ...
                                <a :href="'/admin/product/add'" class="no-underline text-white px-4 my-3 mx-1 flex items-center custom-green py-1 justify-center cursor-pointer rounded">
  ...
  <thead>
    ...
    <!--   <th class="text-left text-sm px-2 py-2 text-grey-darker">Specification</th> -->
      <th class="text-left text-sm px-2 py-2 text-grey-darker">Category</th>
      <th class="text-left text-sm px-2 py-2 text-grey-darker">Vendor</th>
      <th class="text-left text-sm px-2 py-2 text-grey-darker">Code Prefix</th>
       <th class="text-left text-sm px-2 py-2 text-grey-darker">Quantity</th>
       <th class="text-left text-sm px-2 py-2 text-grey-darker">Purchased Date</th>
      <!--  <th class="text-left text-sm px-2 py-2 text-grey-darker">Cost Per Quantity</th> -->
      <!--  <th class="text-left text-sm px-2 py-2 text-grey-darker">Depreciation Rate</th> -->
       <th class="text-left text-sm px-2 py-2 text-grey-darker">Remarks</th>
       <th class="text-left text-sm px-2 py-2 text-grey-darker">In Use</th>
      <!--  <th class="text-left text-sm px-2 py-2 text-grey-darker">Bill Scan</th> -->
      
       <th class="text-left text-sm px-2 py-2 text-grey-darker">Ownership Tracking</th>
       <th class="text-left text-sm px-2 py-2 text-grey-darker">Location Tracking</th>
       <th class="text-left text-sm px-2 py-2 text-grey-darker">Maintainence Tracking</th>
       <th>Action</th>
     </tr>
     </thead>
     <tbody v-if="this.product != ''">
     <tr v-for="products in product">
      <td><a :href="url+'/admin/product/'+products.id+'/show'">{{products.name}}</a></td>
    <!--   <td>{{products.specification}}</td> -->
      <td v-if="products.category!=null">{{products.category.name}}</td><td v-else="">--</td>
      <td v-if="products.vendor!=null">{{products.vendor.name}}</td><td v-else="">--</td>
      <td>{{products.unique_code_prefix}}</td>
      <td>{{products.quantity}}</td>
      <td>{{products.purchased_date}}</td>
     <!--  <td>{{products.cost_per_quantity}}</td>
      <td>{{products.depreciation_rate}}</td> -->
       <td>{{products.remarks}}</td>
      <td>
         <span v-if="products.in_use==1">
     ...
     <!--  <td>{{products.bill_scan}}</td> -->
     
      <td> 
        <span v-if="products.ownership_tracking==1">
          <a >
        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" xml:space="preserve" class="w-5 h-5 fill-current text-green-600 mx-auto"><g><g><path d="M383.841,171.838c-7.881-8.31-21.02-8.676-29.343-0.775L221.987,296.732l-63.204-64.893
         c-8.005-8.213-21.13-8.393-29.35-0.387c-8.213,7.998-8.386,21.137-0.388,29.35l77.492,79.561
         c4.061,4.172,9.458,6.275,14.869,6.275c5.134,0,10.268-1.896,14.288-5.694l147.373-139.762
      ...
      <td>
        <span v-if="products.location_tracking==1">
          <a >
        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" xml:space="preserve" class="w-5 h-5 fill-current text-green-600 mx-auto"><g><g><path d="M383.841,171.838c-7.881-8.31-21.02-8.676-29.343-0.775L221.987,296.732l-63.204-64.893
         c-8.005-8.213-21.13-8.393-29.35-0.387c-8.213,7.998-8.386,21.137-0.388,29.35l77.492,79.561
         c4.061,4.172,9.458,6.275,14.869,6.275c5.134,0,10.268-1.896,14.288-5.694l147.373-139.762
      ...
      <td>
        <span v-if="products.maintainence_tracking==1">
          <a  >
        <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" xml:space="preserve" class="w-5 h-5 fill-current text-green-600 mx-auto"><g><g><path d="M383.841,171.838c-7.881-8.31-21.02-8.676-29.343-0.775L221.987,296.732l-63.204-64.893
         c-8.005-8.213-21.13-8.393-29.35-0.387c-8.213,7.998-8.386,21.137-0.388,29.35l77.492,79.561
         c4.061,4.172,9.458,6.275,14.869,6.275c5.134,0,10.268-1.896,14.288-5.694l147.373-139.762
      ...
      <td>
       <div class="flex items-center">                
              <a :href="url+'/admin/product/'+products.id+'/edit'" class="cursor-pointer" title="Edit">
    ...
    C443.754,73.631,440.467,81.575,434.603,87.419z"></path></g></g></svg>
                  ...
                  <a :href="url+'/admin/product/'+products.id+'/show'"  class="cursor-pointer" title="Show">
                  ...
                  <a @click="deleteproduct(products.id)" class="cursor-pointer" title="Delete">
                   <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512 512" xml:space="preserve" class="w-4 h-4 fill-current text-black-500 mx-1"><g><g><g><polygon points="353.574,176.526 313.496,175.056 304.807,412.34 344.885,413.804       "></polygon> <rect x="235.948" y="175.791" width="40.104" height="237.285"></rect> <polygon points="207.186,412.334 198.497,175.049 158.419,176.52 167.109,413.804       "></polygon> <path d="M17.379,76.867v40.104h41.789L92.32,493.706C93.229,504.059,101.899,512,112.292,512h286.74
...
<script>
 ...
 methods:{
  ...
  productshow(page=1){
            axios.get('/admin/product/list?page='+this.page+'&search='+this.search).then(response=>{
    ...
    deleteproduct(id) 
    {
      ...
      swal({
        ...
        if (isConfirm) 
        {
          axios.delete('/admin/product/'+id+'/delete').then(response => {
             thisswal.success    = response.data.message;
             // thisswal.getlist();
             thisswal.productshow();
             setTimeout(() => {
              thisswal.success = null;
          }, 3000);
          });  
        }
        else 
