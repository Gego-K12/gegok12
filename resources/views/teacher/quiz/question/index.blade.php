...
@section('content')
	<div>
		<h1 class="admin-h1 my-3 flex items-center">
			<a href="{{url('teacher/quiz')}}" title="Back" class="rounded-full bg-gray-100 p-2">
    ...
    <div class="bg-white shadow my-5">
		<div class="flex border shadow py-3 px-5 text-white justify-between items-center">
			<div class="flex-col">
				<p class="text-gray-700 text-2xl font-medium">{{ ucfirst($topic->name) }}</p>
			</div>
			<div class="flex">
				<div class="flex-col leading-relaxed">
					<p class="text-gray-700 font-medium">Top score:<span class="font-semibold">{{ $topic->getTopScore() }} </span></p>
					<p class="text-gray-700 font-medium">Top scorer:<span class="font-semibold">{{ $topic->getTopScorer() }} </span></p>
				</div>
			</div>
		</div>
	</div>
  	...
  	<div class="bg-white shadow my-5">
   		<quiz-tab url="{{ url('/') }}" quiz_topic="{{ $topic->id }}" question_count="{{ count($topic->quizquestions) }}" participant_count="{{ count($topic->quiztests) }}" assign_count="{{ count($topic->quizparticipants) }}"></quiz-tab>         
